/**
 * Tests for the local-state reconciliation bug fixes:
 *
 *  1. Multi-char typing: typing "hello" into a scalar arg must yield "hello",
 *     not just the last character ("o"). Each keystroke previously triggered a
 *     full parse→print→editor→reparse cycle that reset local state, dropping
 *     all but the final character.
 *
 *  2. Add list item persists: clicking "Add item" on a list arg must render a
 *     visible input that stays on screen. Previously the empty item was omitted
 *     from the document (the "empty removes the arg" rule), so the round-trip
 *     removed it before the user could type anything.
 *
 * Both tests use the stateful __state mock so updateActiveTabValues feeds back
 * into the component (the same pattern as query-builder-integration.test.tsx).
 */
...
describe('multi-char scalar input — local state fix', () => {
  ...
  it('typing a multi-char string produces the full value in the document', async () => {
    ...
    const last = writtenQueries.at(-1) ?? '';
    expect(last).toMatch(/query: "hello"/);
  });
  ...
  it('pasting a multi-char string (single change event) produces the full value', async () => {
    ...
    const last = writtenQueries.at(-1) ?? '';
    expect(last).toMatch(/query: "world"/);
  });
