/**
 * Reactive echo-loop harness tests for ArgInput / ListArgInput.
 *
 * These tests model the REAL document round-trip: the harness holds query
 * state, re-parses it on each render to derive the prop value, and feeds the
 * printed result back via `setQuery` — exactly what the QueryBuilder does.
 * The @graphiql/react mock is NOT involved; this tests the reconciliation
 * logic in isolation.
 *
 * Test 1 (regression): clicking "Add item" must NOT cause the new input to
 * vanish after the echo re-render. Before the fix, the echo returned `[]`
 * (empty leaf dropped) which looked like an external change and clobbered
 * local state, removing the just-added empty item.
 *
 * Test 2: multi-char scalar typing accumulates correctly through echoes.
 *
